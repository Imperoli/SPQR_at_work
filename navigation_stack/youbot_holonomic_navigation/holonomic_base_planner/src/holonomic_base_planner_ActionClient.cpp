...
static void goToW1( int event, int x, int y, int, void* )
{
  if( event == EVENT_LBUTTONDOWN )
  {
    geometry_msgs::PoseStamped target_pose;
    target_pose.header.frame_id = "map";
    target_pose.header.stamp = ros::Time::now();
    target_pose.pose.position.x=2.65;
...
static void goToW2( int event, int x, int y, int, void* )
{
  if( event == EVENT_LBUTTONDOWN )
  {
    geometry_msgs::PoseStamped target_pose;
    target_pose.header.frame_id = "map";
    target_pose.header.stamp = ros::Time::now();
    target_pose.pose.position.x=-2.3;
...
int main(int argc, char **argv){
  ...
  //  new windows
  /*namedWindow("WORKSTATION1",CV_WINDOW_AUTOSIZE);
  setMouseCallback( "WORKSTATION1", goToW1, 0 );
  namedWindow("WORKSTATION2",CV_WINDOW_AUTOSIZE);
  setMouseCallback( "WORKSTATION2", goToW2, 0 );*/
